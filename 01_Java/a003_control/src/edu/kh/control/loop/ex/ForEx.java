...
public class ForEx {
	...
	/*
	 * for문 - 끝이 정해져 있는 경우에 사용하는 반복문 (== 반복 횟수가 지정되어 있느 ㄴ경우 사용)
	 * 
	 * - 작성법 [순서] for( 초기식[1] int a = 1; 조건식[2(true)] a<3 [5(true)] a<3; 증감식[4] a++
	 * [7]; ) {
	 * 
	 ...
	 * 
	 * - 초기식 : for문을 제어하는 용도의 변수 선언 및 초기화 (변수에 값 처음 대입)
	 *
	 * - 조건식 : for문의 반복 여부를 지정하는 식 조건식이 true인 경우에만 반복 수행을 함
	 * 
	...
	public void ex2() {
		...
		// 5부터 12까지 1씩 증가하며 출력
		for (int num = 5; 5 <= num && num <= 12; num++) {
	...
	public void ex9() {
		...
		for (int i = 1; i <= num; i++) {
			...
			// int num2 = sc.nextInt(); 변수2 선언 생략(중복되니까)
			sum += sc.nextInt();
		}
	...
	public void ex14() {
		...
		/*
		 * for (int i = 1; i <= 9; i++) { if (num >= 2 && num <= 9) {
		 * System.out.printf("%d x %d = %2d\n", num, i, num * i); } else {
		 * System.out.println("잘못 입력하셨습니다."); } }
		 */
	...
	public void ex15() {
		...
		for (int x = 1; x <= 4; x++) { // 생각3) 4회 반복
			for (int i = 1; i <= 5; i++) { // 생각1) 12345 출력
				System.out.print(i);
			}
			System.out.println(x); // 생각2) 줄바꿈(개행)
		}
	...
	public void ex17() {
		...
		for (int dan = 2; dan <= 9; dan++) {
			...
			System.out.println(); // 안쪽 구문 끝났을 때 개행해주기
		}
	...
	public void ex22() {
		...
		int count = 1; // 화면에 1부터 시작하니까 // 숫자로 세기 위한 변수 선언
