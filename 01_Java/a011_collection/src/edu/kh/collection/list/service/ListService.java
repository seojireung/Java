...
public class ListService {
	 ...
	 * [특징]
	 * - 인덱스 이용해서 순서 유지
	 * - 순서 구분 가능 -> 중복 데이터 저장 가능
	 * 
	 * List 인터페이스를 구현한 대표적인 클래스
	 * -> ArrayList(최신), Vector(옛), LinkedList
	  */
...
//	list.add(10, "외딴섬"); // 오류 발생
	
	// E get(int index) : 주어진 index의 요소를 반환
//	System.out.println(list.get(0));
	
	// int size() : 리스트 크기x, 리스트에 저장된 요소의 개수
	for(int i=0; i<list.size(); i++) {
		System.out.println(list.get(i));
		
		// * 컬렉션에서 여러 데이터 타입을 저장할 경우의 단점 * *****다시
		// - 각 요소마다 타입이 다르기 때문에
		//   원하는 코드가 있을 경우 타입 검사 + 다운캐스팅이 강제 필요.
