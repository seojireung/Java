...
// * (애스터리스크): 모든, ALL
// dto 패키지의 모든 클래스 import 하겠다는 의미
...
//		Animal a = new Animal();
		...
		// 3. 추상 클래스는 객체 생성x / 하지만 부모 타입 참조 변수 o
		// (다형성의 업캐스팅)
	...
	public void ex2() {
		...
		// 2. 인터페이스로 객체 생성 x, 부모 타입 참조 변수는 가능
//		KH k = new KH(); // 객체 생성 x
		
		// 객체 배열 + 다형성 + 동적바인딩
		KH[] arr = new KH[2]; // KH 참조 변수의 묶음
...
//		Calculator cal = new BDHCalculator();
		...
		// 수정할 때 뒤 객체만 바꾸고 아래 코드들은 수정할 필요 없기 때문에
		// 유지 보수 용이함
