...
public class Student {
	...
	// Object.equals() 오버라이딩
	// -> 현재 객체(여기 클래스)와 다른 객체의 동등(필드 값이 같은지) 비교
	// -> 필드를 가지고 있는 자식 클래스가 
	//    알맞은 형태로 재정의(오버라이딩)
	@Override
	...
	// - hashCode() : 객체의 필드 값ㅇ르 이용해서
    //	              일정한 길이의 숫자를 만드는 함수
	
	// 왜 필요한가? Java 실행 시 내부에서 객체를 검색하는 속도 증가, 중복 제거
	
	// 어떻게 작성하는가?
	// 필드 값이 같으면 항상 같은 수가 나올 수 있도록 (어떻게든) 구현
	// + equals() 오버라이딩 시 필수적으로 같이 오버라이딩
	
	@Override
	public int hashCode() {
		int result = 1;
		final int PRIME = 31; // 겹치지 않도록 적당히 큰 소수 사용 (곱연산 시 속도 빠른 수)
