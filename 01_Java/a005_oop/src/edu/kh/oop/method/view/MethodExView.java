...
public class MethodExView {
	...
	public void displayMenu() { // 메뉴 표시(출력) 기능
		...
		do {
			...
			switch(input) {
			case 1 : menu1(); break; // 1 입력 시 menu1() 메서드 실행
			case 2 : menu2(); break;
			case 3 : menu3(); break;
			case 4 : menu4(); break;
	...
	private void menu2() {
		...
		// 정수 1,2,3에 각각 num 1,2,3를 전달함
		service.threeNumbersSumAndAverage(num1, num2, num3);  // 호출
	...
	private void menu3() {
			...
			int[] numbers = service.fiveRandonNumbers(); // 1. 호출
			// 1) service의 fiveRandonNumbers()를 호출
			// 2) 난수 5개가 저장된 int[]이 반환됨(돌아옴)
			//    -> 반환된 int[]은 실제 배열이 아닌
			//		 int[]의 주소값이다. (== 얕은 복사 개념 적용)
			// 3) 반환된 int[] 배열을 numbers 참조 변수에 저장 (반환형 같음)
