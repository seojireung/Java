...
//		RequestDispatcher dispatcher = req.getRequestDispatcher("wepapp 폴더를 기준으로 jsp 경로 작성")
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/scope/scope.jsp");
		
		// 1. page scope -> JSP에서 확인
		
		// 2. request scope -> 요청 받은 페이지(servlet) + 위임 받은 페이지(jsp)
		...
		// -> request, session 객체에서 얻어오기 가능(2개 결과 같음)
