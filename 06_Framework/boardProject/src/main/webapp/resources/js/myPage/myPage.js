...
if(updateInfo != null){
    ...
    memberTel.addEventListener("input",()=>{
        ...
        if(memberTel.value == initTel){
            membeTel.removeAttribute("style");
            return;
        }
...
// 비밀번호 변경 제출 시
const changeFrm = document.getElementById("changePwFrm");
...
if(changeFrm != null){ // 비밀번호 변경 페이지인 경우
...
if(imageInput != null){ // 화면에 imageInput이 있을 경우
    ...
    //  -> 이 때 input값 입력 후 포커스를 잃었을 때(포커스 다른 곳에)
    ...
    imageInput.addEventListener("change", e => {
        ...
        console.log(e.target.files); // 업로드된 파일의 정보가 담긴 배열(여러 장일 수 있으니까)
        ...
        if(file.size > maxSize){
            ...
            imageInput.value = ""; // 빈칸 밖에 못씀.. 경로 직접 적을 수 x
