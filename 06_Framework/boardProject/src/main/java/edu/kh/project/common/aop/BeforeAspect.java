...
//  execution( [접근제한자(생략)] *(리턴타입)  클래스명   *(메소드명) (..)([파라미터]) )
//	@Before("execution(* edu.kh.project..*Impl*.*(..))") ""내용 -> @Pointcut으로 옮김
	
	// 타겟 수행 전
	// 지정된 클래스.메서드에 작성된 @Pointcut() 어노테이션의 내용을 타겟으로 삼음 (변수 선언과 같은 맥락)
	...
	public void beforeLog(JoinPoint jp) { // Advice(수행할 코드)
		...
		// 메서드 선언부(== 메서드 signature)에서 메서드명만 얻어옴
