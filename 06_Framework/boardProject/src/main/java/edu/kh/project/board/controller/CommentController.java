...
public class CommentController {
	...
	public List<Comment> select(int boardNo) { // 파라미터 가져오기위해 @RequestParam
		return service.select(boardNo); // HttpMessageConverter List -> JSON 변환
	}
	
	// 댓글 삽입
	@PostMapping("/comment")
	public int insert(@RequestBody Comment comment) { // 값이 body에 담겨 있음
		// 요청 데이터(JSON)을 
		// HttpMessageConverter가 해석해서 Java 객체(comment)에 알아서 필드 대입.
