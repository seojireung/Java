...
@Component // @Controller, @Service, @Repository의 부모 어노테이션 (부모 == 자식의 공통분모)
		   // Bean 등록을 하겠다고 명시하는 어노테이션
public class ImageDeleteScheduling {
	
	@Autowired
	private ServletContext servletContext; // application scope
	
	@Autowired
	private BoardService service;
	
	// 3순위
	//@Scheduled(fixedDelay = 10000) // ms 단위 (일 끝난 후 대기 시간)
	// 일(5초) -> 10초 대기 -> 일(5초) -> 10초 대기
	
	// 2
	//@Scheduled(fixedRate = 10000) // ms 단위 (일 시작하고 다음 일 시작 사이 시간)
	// 일(5초)
	// 대기(10초)
...
//		System.out.println("스케쥴러가 일정 시간마다 자동으로 출력");
		...
		// 1) 서버에 저장된 파일 목록 조회
		// (servletContext(application scope) 의존성 주입)
