...
--> 테이블 정보에서 컬럼명, COMMENT 확인 (ctrl+클릭)
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 급여 + 100만원 조회
...
-- EMPLOYEE 테이블에서 현재 사원들이 속해 있는 부서 코드 종류 조회
...
--         같은 문자이기 때문에 구분 되지 않는 문제 발생 (시험 문제로 딱!)
...
/* (주의) WHERE절에서 별칭|순서는 사용할 수 없다 _ 해석 순서 때문에 */
