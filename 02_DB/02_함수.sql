...
--------------------------------------------
-- INSTR(문자열|컬럼명, '찾을 문자열' [, 찾을 시작 위치 [, 순번]]) (INNNERSTRING)
-- 찾을 시작 위치부터 지정된 순번째에 찾을 문자열의 시작 위치(인덱스) 반환
...
SELECT EMP_NAME, SUBSTR(EMAIL, 1,INSTR(EMAIL,'@')-1) 아이디 /*'-1'의 위치 주의!*/
FROM EMPLOYEE
ORDER BY 아이디;
...
SELECT 123.456,
	...
	ROUND(123.456, 1), -- 소수점 첫째 자리까지 반올림
	ROUND(123.456, 2), -- 소수점 둘째 자리까지 반올림
	ROUND(123.456, 0), -- 소수점 0번째 자리까지 반올림 -> 정수
...
-- SYSTIMESTAMP : SYSDATE반환값 + MS 단위 추가
SELECT SYSDATE, SYSTIMESTAMP FROM DUAL;
--2023-03-07 12:23:11.000, 2023-03-07 12:23:11.188 +0900
...
FROM DUAL; --2023-02-07 12:37:53.000, 2023-03-07 12:37:53.000, 2023-04-07 12:37:53.000

-- LAST_DAY(날짜) : 해당 월의 마지막 날짜를 반환
SELECT LAST_DAY(SYSDATE), LAST_DAY('2023/04/01') 
FROM DUAL; --2023-04-30 00:00:00.000
...
FROM EMPLOYEE;
--> 숫자를 문자열로 바꿀 떄 칸 수가 충분하지 않으면 '#'으로 채워짐
...
-- DECODE(계산식 | 컬럼명, 조건1, 결과1, 조건2, 결과2...[, 아무것도 일치 X])

-- 비교하는 식 또는 컬럼의 값이 일치하는 조건이 있으면
-- 해당 조건의 결과를 반환
-- 만약 일치하는 경우가 없으면 제일 끝 값 반환
-- 자바의 switch문과 유사함
...
SELECT EMP_ID, EMP_NAME, SALARY,
	...
	END 구분
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6' ORDER BY JOB_CODE /*ASC*/;
...
-- COUNT(컬럼명)	   : 지정된 컬럼의 값이 NULL인 경우를 제외한 행의 개수
-- COUNT(DISTINCT 컬럼명)
--  : 지정된 컬럼에서 중복된 값을 제외한 행의 개수 반환
