...
-- 에러: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다.

SELECT DEPT_CODE, SUM(SALARY) --1) 그룹별 SALARY 합계를 조회
FROM EMPLOYEE --2) EMPLOYEE 테이블에서
GROUP BY DEPT_CODE; --3) DEPT_CODE가 같은 행끼리 그룹을 지어
 ...
 * 4 : HAVING 그룹함수식을 이용한 조건식 (GROUP BY가 있어야 작성 가능!)(그룹을 걸러냄)
...
-- 부서코드 오름차순 조회
SELECT DEPT_CODE, SUM(SALARY), ROUND(AVG(SALARY)), 
	COUNT(*), MIN(HIRE_DATE), MAX(HIRE_DATE)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
-- EMPLOYEE 테이블에서
-- 성별과, 성별 별 급여 평균(내림 처리), 급여 합, 인원 수 조회
-- 인원 수 오름차순 정렬
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별,
	FLOOR(AVG(SALARY)) "급여 평균", SUM(SALARY) "급여 합 ",
	COUNT(*) "인원 수"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') --★함수 사용★
...
-- EMPLOYEE 테이블에서 직급별 2000년도 이후(이상) 입사자들의 급여 합 조회
...
/*3*/HAVING AVG(SALARY) >= 3000000 -- GROUP BY에서 묶인 그룹에 조건 대입
/*5*/ORDER BY 1;

-- 부서별 급여가 300만원 이상인 사원들의 평균 급여를 조회
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
WHERE SALARY >= 3000000 -- EMPLOYEE 테이블의 모든 행에 조건 대입
GROUP BY DEPT_CODE 
ORDER BY 1;
