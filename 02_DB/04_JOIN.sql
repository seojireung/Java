...
--> NATURAL JOIN(아래 6번) 실패 시 결과로 확인 가능
...
-- 반드시 두 테이블 간의 동일한 컬럼명, (데이터) 타입을 가진 컬럼이 필요
--> 없을 경우 교차조인이 됨.
...
-- DEPARTMENT : DEPT_ID   CHAR(2) D1 ~ D9		...타입은 같으나 컬럼명 다름
---------------------------------------------------------------------------------------------------------------
...
--연봉에 보너스포인트를 적용하시오.(연봉=(급여*(1+보너스))*12)
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY*(1+BONUS))*12
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN SAL_GRADE USING (SAL_LEVEL)
GROUP BY SAL_LEVEL
HAVING SALARY > MIN_SAL; --다시
...
-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 같은 부서의 동료이름을 조회하시오.
-- 동료==같은 부서==같은 부서코드
-- SELF JOIN 사용
SELECT 
FROM
