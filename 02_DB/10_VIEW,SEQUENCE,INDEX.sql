 ...
 * 
 *  * 테이블 : 행과 열로 이루어진 데이터 베이스의 기본적인 객체
 * 
 ...
 *  AS 서브쿼리(다른 SQL문 안에 포함된 또다른 SQL문)
 *  [WITH CHECK OPTION]
 *  [WITH READ ONLY];
 * 
 * 1) OR REPLACE 옵션 : 
 * 		기존에 동일한 이름의 VIEW가 존재하면 이름 *변경*
 * 		없으면 새로 생성.
 * 
 * 2) FORCE | NOFORCE 옵션 : 
 ...
 * 
 * 3) 컬럼 별칭 옵션 : 조회되는 VIEW의 컬럼명 지정 (잘 안씀)
 * 
 * 4) WITH CHECK OPTION 옵션 :
 * 		옵션을 지정한 컬럼의 값을 수정 불가능하게 함.
 * 
 * 5) WITH READ ONLY 옵션 : VIEW에게 SELECT만 가능하도록 지정. (DML 아예 X)(자주 씀)
 * 
 * */
...
--  저장된 것처럼 보임!
--> 실제로 VIEW에 저장된게 아니라
--  VIEW를 생성 할 때 사용한 서브쿼리의 테이블(DEPT_COPY2)에
--  값이 삽입되어 있다!
...
--> *원본 테이블* DEPT_ID, LOCATION_ID 컬럼에 'D0', 'L3' 삽입해라.
--   DEPT_TITLE 컬럼에는 NULL 삽입
---> VIEW 이름 적었지만 원본 테이블에 삽입하라는 뜻
----> 그런데.. DEPT_TITLE에는 NOT NULL 제약조건 설정되어
--	  NULL 값이 삽입될 수 없음.

-- ORA-01400: NULL을 ("KH_SJY"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- 데이터 무결성 : 중복 X, NULL X -> 신뢰도가 높은 데이터
--> 대부분의 컬럼에 NOT NULL 제약조건 추가되어 있음
---> NOT NULL이 설저된 테이블을 이용해서 VIEW를 만들면 
--   INSERT가 거의 불가능.

--> 결론 : VIEW 가지고 DML 수행 지양하자!
 ...
 * [작성법]
  ...
  -(↓옵션↓)-
  [STRAT WITH 숫자] -- 몇부터 시작할래? 처음 발생시킬 시작값 지정,
  					  생략하면 자동 1이 기본
 ...
 * 	  NEXT VALUE	   (INCREMENT BY 만큼 증가된 수)
 * 					   단, 생성 후 처음 호출된 시퀀스인 경우
 * 					   START WITH에 작성된 값 반환.
 * 
 * 2) 시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴.
 * 	  CURRENT VALUE	   단, 시퀀스가 생성 되자마자 호출할 경우 오류 발생
...
-- ※ 실수로 더 생성했으면 되돌릴 수 없고 다시 만들어야 함.
 ...
 *   컬럼에 대하여 생성하는 객체
 * 
 * - 인덱스 내부 구조는 B* 트리 형식으로 되어있음. (B: Binary 이진수 / Balanced) (이진트리)
 *    
 * ** INDEX의 장점 **
 ...
 * - 인덱스도 하나의 객체이다 보니 별도 저장공간이 필요 (= 메모리를 소비한다)
 * - 인덱스 생성 시간 필요
 * 
 * [작성법]
 ...
 * ON 테이블명 (컬럼명[, 컬럼명 | 함수명]);
 * 
 * (옵션) UNIQUE : 중복 없는 컬럼에 인덱스 생성
 * 
 * (삭제) DROP INDEX 인덱스명;
 * 
 * ** 인덱스가 자동 생성되는 경우 **
 * -> PK 또는 UNIQUE 제약조건이 설정된 컬럼에 대해
 *    UNIQUE INDEX가 자동 생성됨.
 * */
...
-- (PL/SQL 사용 _ SQL 이용해서 자바처럼 코드 짜기)
BEGIN
	FOR I IN 1..1000000
	LOOP
...
WHERE TEST_ID = 'TEST500000'; -- 38 ms

-- 인덱스 사용 O 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = '500000'; -- 1 ms
...
-- 인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 5/1/0 ms
...
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 다시 느려짐
