/*
- 데이터 딕셔너리란?
DDMS 자원을 효율적으로 관리하기 위한 다양한 정보를 저장하는 시스템 테이블
...
SELECT * FROM User_tables; --DDMS 시스템이 알아서 만들어놨음.
...
/* 자료형 (추린 것)
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE) 
        ...
        자바 CHAR(문자 하나-DB엔 없는 개념)랑 다름. DB에서 CHAR = JAVA에서 STRING.
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE)
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        -> 좀 더 오래 걸리지만, 문자열 길이 예측 어려울 때 메모리 관리에 좋음.
        
    ※ UTF-8에서 문자(한글/영어) 한글자에 3BYTE 차지, 적은 용량 문제 해결 위해 ↓
        
   참 DATE : 날짜 타입
   고 BLOB : 대용량 이진 데이터 (4GB) 
   만 CLOB : 대용량 문자 데이터 (4GB)
*/
...
CREATE TABLE MEMBER(
	...
	MEMBER_NAME VARCHAR2(15), -- 한글 2글자~5글자(3*5 BYTE)
	MEMBER_SSN  CHAR(14),     -- 주민번호 : 고정된 14글자
...
INSERT INTO MEMBER2 VALUES('김길동', 0001011234567, 01012341234);

SELECT * FROM MEMBER2;
-- 9001011234567  1012341234 
--    1011234567  1012341234
...
/*
    ...
    *****데이터 무결(결함X)성 보장을 목적으로 함.*****
    ...
    제약조건 종류 (무조건 암기!!!!!!)
    ★1 PRIMARY KEY, NOT NULL, UNIQUE, CHECK, 2 FOREIGN KEY.★
...
CREATE TABLE USER_USED_NN( --NN: NOT NULL 의미
...
-- ORA-01400: NULL을 ("KH_SJY"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- UNIQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK( --UK: UNIQUE KEY
    USER_NO NUMBER,
    
--  USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건 설정(이름 미지정)
...
-- ORA-00001: 무결성 제약 조건(KH_SJY.SYS_C007440)에 위배됩니다
-- ORA-00001: 무결성 제약 조건(KH_SJY.USER_ID_UK)에 위배됩니다
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> ORA-00001: 무결성 제약 조건(KH_SJY.USER_ID_NAME_UK)에 위배됩니다
...
----------------------------------------------------------------------------------------------------------------
--★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
-- 3. PRIMARY KEY(기본키) 제약조건 'PK' (너무 중요해서 기본키)

SELECT * FROM EMPLOYEE; -- EMP_ID가 PK

-- 모든 테이블에 PRIMARY KEY으로 지정된 컬럼 있어야 함.
-- 테이블에서 한 행의 정보를 찾기위해 사용할 컬럼을 의미함
-- 테이블에 대한 *식별자(IDENTIFIER) 역할*을 함
--> NOT NULL + UNIQUE 제약조건의 의미만!! 제약조건 작성한 건 아님!!
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_SJY.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_SJY"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
SELECT * FROM USER_USED_PK2; -- 3행 조회
...
-- FOREIGN KEY제약조건에 의해서 테이블간의 *관계(RELATIONSHIP)(=선)*가 형성됨
...
CREATE TABLE USER_USED_FK(
  ...
  --> 작성법 *REFERENCES* (FOREIGN KEY 아님!) .. 문제해결 시험..?
...
--> NULL(외부 테이블에 참조할 값이 없다, 참조X) 사용 가능.
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- ORA-02291: 무결성 제약조건(KH_SJY.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
 --> 참조할 부모 키가 없음 (FK가 자식 개념)
...
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KH_SJY.GRADE_CODE_FK1)이 위배되었습니다
--  - 자식 레코드(기록)가 발견되었습니다
...
--  CONSTRAINT 제약조건명 FOREIGN KEY(컬럼명)
  , CONSTRAINT GRADE_CODE_FK2 FOREIGN KEY(GRADE_CODE) -- 테이블레벨은 'FOREIGN KEY' 써야됨!!!
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_NAME VARCHAR2(30), 
  -- 컬럼 레벨
  GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK (GENDER IN ('남', '여')),-- 남, 여 제한 / IN: 다중 비교
...
VALUES (1,'user01','pass01','980119-2000000','여','010-9876-5432','서울시 종로구','N');
INSERT INTO USER_TEST
VALUES (2,'USER02','PASS02','980119-1000000','남','010-9876-5432','서울시 노원구','Y');
INSERT INTO USER_TEST
VALUES (3,'USER03','PASS03','990112-2000000','여','010-9876-5432','경기도 남양주시','N');
INSERT INTO USER_TEST
VALUES (4,'USER04','PASS04','990112-1000000','남','010-9876-5432','서울시 마포구','Y');
INSERT INTO USER_TEST
VALUES (5,'USER05','PASS05','970330-2000000','여','010-9876-5432','전주시','N');
...
--AS 서브쿼리; 
--서브쿼리의 결과 (RESULT SET)의 형태로 테이블 생성
...
-- 3) 테이블의 형태 (컬럼명, 데이터 타입)만 복사 (컬럼값 복사X)
CREATE TABLE DEPARTMENT_COPY AS
SELECT * FROM DEPARTMENT 
WHERE 1 = 0; -- 임의로 FALSE 만들어서 모든 컬럼값 복사 안되게 의도.
