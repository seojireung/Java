...
--          ㄴ 사용 시 주희사항! ORDER BY절 해석 전에 먼저 행번호를 부여함
SELECT /*ROWNUM,*/ EMP_ID
FROM(SELECT EMP_ID FROM EMPLOYEE ORDER BY SALARY)
WHERE ROWNUM = 1;

/*ROWNUM 사용 주의사항*/
--SELLECT ROWNUM, EMP_ID, SALARY
--FROM EMPLOYEE
--ORDER BY SALARY;

--> 확실한 방법(복잡 하지만 신뢰도 있는 정보 조회)
...
SELECT DEPT_TITLE/*23행*/, SUM(SALARY)/*1행*/
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE /*DEPT_CODE를 묶으면 RESULT SET이 직사각형 모양이 아님 */
...
-- 1) 사수에 해당하는 사원 번호 조회
SELECT DISTINCT/*중복 삭제*/ MANAGER_ID 
FROM EMPLOYEE
...
NATURAL JOIN JOB
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE (DEPT_TITLE, JOB_NAME) IN (SELECT DEPT_TITLE, JOB_NAME
					FROM EMPLOYEE
					NATURAL JOIN JOB
					JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
					WHERE ENT_YN = 'Y'
					AND SUBSTR(EMP_NO, 8, 1) = '2');
 -- 다중열만큼 WHERE 컬럼 개수 작성.
...
WHERE (DEPT_CODE, JOB_NAME) 
 = (SELECT DEPT_CODE, JOB_CODE 
	FROM EMPLOYEE
	WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000); --********날짜 추출 복습********
...
WHERE (JOB_CODE, SALARY) IN (SELECT JOB_CODE, TRUNC(AVG(SALARY),-4)
						     FROM EMPLOYEE
							 GROUP BY JOB_CODE);
-- >,< 등 대소비교연산자는 숫자만 비교 가능.
                  
-------------------------------------------------------------------------------

-- 5. ★상[호연]관 서브쿼리★ 어려움
-- 위에 까지 서브쿼리는 해석 순서 서브 → 메인쿼리 순.
-- 상관 서브쿼리는 해석 순서 (메인쿼리 각 행 마다 메인→서브→메인)*행(ROW)의 수 만큼 반복 => 하나의 결과 산출.
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE /**/ IN (SELECT MIN(HIRE_DATE)
		  FROM EMPLOYEE SUB
		  WHERE MAIN.HIRE_DATE = SUB.HIRE_DATE 
		  AND ENT_YN = 'N'
		  GROUP BY DEPT_CODE
		 )
ORDER BY HIRE_DATE;

--1) 간단하지만 100% 신뢰할 수 없음. (날짜가 중복되면 잘못 불러올 수 있음)
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE, DEPT_CODE 
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
...
--	  번호 1번부터 매김, 해석 순서 SELECT 보다 우선.

--4) 1,2,3 번을 토대로 급여 상위 5명을 조회 시도
SELECT EMP_NAME, SALARY, ROWNUM
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC; 
--> 마지막으로 해석, EMPLOYEE 테이블 위에서부터 5명 순서(+급여 내림차순)일 뿐
...
FROM (SELECT EMP_NAME, SALARY 
  	  FROM EMPLOYEE
	  ORDER BY SALARY DESC) --> 메인쿼리에 포함된 뷰 생성 구문
	  						--> 인라인 뷰
WHERE ROWNUM <= 5;
...
-- 8. WITH (서브쿼리 따로 어디 써놓고 그거 갖다 쓰겠다.)
