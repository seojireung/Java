...
-- ALTER(바꾸다, 변조하다)
-- 수정 가능한 것 : 컬럼(추가/수정(컬럼명,데이터 타입 등)/삭제), 제약조건(추가/삭제)(제약조건 수정구문은 없음)
--               이름변경(테이블, 컬럼, 제약조건)
...
-->  NOT NULL은 기존 컬럼의 *성질을 변경하는 것*(ON/OFF 정도..)으로 인식됨.(MODIFY)
    --> 컬럼의 간단한 스위치라 여기고 컬럼에 제약조건 직접 작성. (테이블에서 X)

--ALTER TABLE DEPT_COPY ADD
--NOT NULL(DEPT_TITLE); -- 테이블에서 안됨

ALTER TABLE DEPT_COPY MODIFY
DEPT_TITLE NOT NULL; -- NOT NULL 관련 설정만 MODIFY 사용!! (기억!)
---------------------------
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007507; --NOT NULL
--> NOT NULL 제약조건 ADD는 안되는데 DROP은 가능

-- NOT NULL 제거 시 MODIFY 사용도 가능
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE CONSTRAINT SYS_C007507 /*NOT*/ NULL;
...
ALTER TABLE DEPT_COPY 
MODIFY DEPT_ TITLE VARCHAR2(10);
...
--추가 설명
--DML이 트랜잭션에 머문 상태에서 DDL을 커밋하면
--디비버가 트랜잭션 같이 밀고 저장하기 때문에
--트랜잭션의 ROLLBACK 불가능함 (반영이 안됨)
