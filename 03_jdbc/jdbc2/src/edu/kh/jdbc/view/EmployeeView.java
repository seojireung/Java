...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				// 확인) SELECT EMP_ID, EMP_NAME, ENT_YN, ENT_DATE FROM EMPLOYEE WHERE EMP_ID = 입력한 사번
	...
	} catch (SQLException e) { // throws 이용해서 예외처리 view에서 하는 이유 :
							   // 입/출력 view에서만 하기 위해서.
		System.out.println("\n[사원 전체 정보 조회 중 예외 발생]\n");
		e.printStackTrace();
	}
	...
	private void selectName() {
		...
		try {
			List<Employee> empList = service.selectName(words);
			
			if(empList.isEmpty()) { // 글자 포함하는 이름 x 경우
	...
	private void selectSalary() {
		...
		try {
			List<Employee> empList = service.selectSalary(minSal, maxSal);
			
			if(empList.isEmpty()) {
				System.out.println("[입력하신 범위 내 급여에 해당하는 사원이 없습니다.]");
				return;
			}
			// 조회된 결과가 N행일 경우
			for(Employee emp : empList) {
