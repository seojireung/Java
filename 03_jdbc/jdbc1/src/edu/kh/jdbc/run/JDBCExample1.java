...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// DB *연결 정보를 담는* 객체 (어떤 DB와 java를 연결하는 통로)
		...
		//    상속을 받아 공통된 부분을 DBMS마다 알맞게 구현하고
		//    구현된 객체를 Java에서 참조해서 사용 
		//	  (다형성 장점 적용)
		
		Statement stmt = null;
		// *sql을 DB에 전달*해 수행하고 
		// 결과(ResultSet 또는 DML 성공 행의 개수)를 *반환 받*는데
		// 사용되는 객체 (sql, (ResultSet 또는 DML 성공 행의 개수) 담는 상자)
		...
		try {
			...
			// -> () 안에 작성된 경로에 존재하는 클래스를 이용해 객체 생성
			...
			// 115.90.212.22 (강의장 DB 서버 이용자 = '나')
			String port = ":1521"; // 컴퓨터 내 응용프로그램 번호
			// :9000 (강의장 DB 서버 이용자 = '나')
			String dbName = ":ORCL"; // DB 이름 (비슷한 단어로 SID)
			// 19c 버전 :             :ORCL
			// 18c 버전 또는 강의장DB :  :XE
			String user = "kh_sjy"; // 계정명
			String pw = "oracle_sjy5599"; // 비밀번호
...
//			System.out.println(conn); 
			...
			// 5. Statement 객체를 통해 SQL(SELECT문)을 수행하고
			//	  결과(ResultSet)를 반환 받아와서 저장(위에 선언한 변수 rs)
			...
			// 첫 번째 행부터 1행씩 접근하여 컬럼 값을 얻어와 출력 (반복)
			// 반복이 언제 끝날지 정해져 있지 않은 while 문 사용
			
			while(rs.next()) {
				// 조건식의 결과가 true일 때 수행
				
				// rs.next()
				// - ResultSet의 객체는 조회 결과를 한번에 테이블로 다루는 것이 아닌
...
//				java.sql.Date hireDate = rs.getDate("HIRE_DATE");
			...
			// [4단계] : 사용한 JDBC 객제 자원 반환(close())
			// -> close 구문은 객체 생성 역순으로 작성! (안전보장)
			
			// 생성 순서 : conn, stmt, rs
			// close 순서: rs, stmt, conn
			
			try {
				if(rs != null) rs.close(); // 처음에 받아오는 과정에서 발생한 오류로 null값일 때 대비.
