...
-- 회원(MEMBER) 테이블
CREATE TABLE "MEMBER"(--테이블 만들 때 테이블명에 "" 표시하는게 좋음
...
COMMENT ON COLUMN "COMMENT"."CREATE_DT" IS '댓글작성일';
...
ORDER BY BOARD_NO DESC; -- 속도 빠르게 하려고 PK이용

-- 댓글 샘플 데이터 3개 삽입
INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '댓글 샘플 1번', DEFAULT, DEFAULT, 7,13);
INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '댓글 샘플 1번', DEFAULT, DEFAULT, 8,13);
INSERT INTO "COMMENT"
VALUES(SEQ_COMMENT_NO.NEXTVAL, '댓글 샘플 1번', DEFAULT, DEFAULT, 9,13);
...
-- 삭제되지 않은 글 중 최신 게시글이 위쪽으로 오도록 정렬
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT,
...
SELECT MEMBER_ID, MEMBER_NM, DECODE(MEMBER_GENDER, 'F','여','M','남') 성별
FROM "MEMBER"
WHERE UNREGISTER_FL = 'N'
...
-- 다음 시퀀스 번호 생성 (있는 거 조회가 아니라 새로 생성이니까 DUAL에서.)
