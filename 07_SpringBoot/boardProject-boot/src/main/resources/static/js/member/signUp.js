...
// JS 객체 : {"K":V, "K":V, "K":V, "K":V 반복} (Map 형식)
// 특징
...
memberEmail.addEventListener("input", () => {
    ...
    if(regEx.test(memberEmail.value)){ // 유효한 경우
        ...
        fetch('/dupCheck/email?email=' + memberEmail.value) // 입력한 이메일을 전달
...
memberPw.addEventListener("input", ()=>{
    ...
    if(regEx.test(memberPw.value)){
        ...
        pwMessage.innerText = "비밀번호 형식이 유효하지입니다";
        pwMessage.classList.add("error");
        pwMessage.classList.remove("confirm");
...
memberNickname.addEventListener("input", ()=>{
    ...
    if(memberNickname.value.trim() == ''){
        ...
        memberNickname.value = ""; // 빈칸 제출 못하게 핟는 코드
        return;
    }
...
checkAuthKeyBtn.addEventListener("click", function(){
    ...
    if(authMin > 0 || authSec > 0){ // 시간 제한이 지나지 않은 경우에만 인증번호 검사 진행
        ...
        // ?inputKey=123456&email=user01 (파라미터가 많을 때 쓰는 인증 방법)
    ...
    for(let key in checkObj){
        ...
        if(!checkObj[key]){ // 각 key에 대한 value(true/false)를 얻어
            ...
            switch(key){ 
                ...
                case "memberNickname":
                    alert("닉네임이 유효하지 않았습니다"); break;
                case "memberTel":
                    alert("전화번호가 유효하지 않았습니다"); break;
                }               
